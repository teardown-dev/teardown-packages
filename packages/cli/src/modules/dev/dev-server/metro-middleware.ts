...
const rawBodyMiddleware: connect.NextHandleFunction = (req, _res, next) => {
	...
	reqWithBody.rawBody = "";
	reqWithBody.on("data", (chunk) => {
		reqWithBody.rawBody = reqWithBody.rawBody + chunk;
	});
	reqWithBody.on("end", next);
};

const metroOpenStackFrameMiddleware: connect.NextHandleFunction = (
	req,
	res,
	next,
) => {
	...
	if (!("rawBody" in req) || !req.rawBody) {
		res.statusCode = 406;
		return res.end(
			"Open stack frame requires the JSON stack frame as request body",
		);
	}
...
function createMetroStatusMiddleware(
	...
	return (_req, res) => {
		if (!metroConfig.projectRoot) {
			res.statusCode = 500;
			return res.end("Project root is not set");
		}
